...
#scipt to take gff of genes and bed files of TE and non-TE repeat locations, and to extract % overlap between full transcript and CDS of each transcript
#requires bedtools
...
while (my $line = <IN1>) {
	chomp $line;
	my @cols = split(/\t/, $line);
	if ( ($line !~ /^#/) and ($cols[2] eq "mRNA") ) {
		$pro = 1;
		my @icols = split(/\;/, $cols[8]);
		$geneID = substr $icols[1], 5;
		open (OUT2, ">temp_transcript.bed") or die;
		open (OUT3, ">temp_CDS.bed") or die;
	}
	elsif ( ($line !~ /^#/) and ($cols[2] =~ /UTR/) and ($pro == 1) ) {
		my $estart = $cols[3] - 1;
		print OUT2 "$cols[0]\t$estart\t$cols[4]\t$geneID\t.\t$cols[6]\n";
	}
	elsif ( ($line !~ /^#/) and ($cols[2] eq "CDS") and ($pro == 1) ) {
		my $cstart = $cols[3] - 1;
		print OUT2 "$cols[0]\t$cstart\t$cols[4]\t$geneID\t.\t$cols[6]\n";
